...
int main(void) {
  int rowmat1, columnmat1, rowmat2, columnmat2, collector, sum = 0;
  ...
  do {
    ...
    if (columnmat1 != rowmat2) {
      printf("ERROR: Doing Matrix Multiplication The Matrix1's Column Must "
             "Equal to The Matrix2's Row.\nPlease Enter Please Enter Sizes of "
             "Matrix 2 Again.\n");
    }
  ...
  for (int row = 0; row < rowmat1; row++) {
    for (int column = 0; column < columnmat2; column++) {
      ...
      for (int time = 0; time < columnmat1; time++) {
        collector = 0;
        if (mat1[row][time] == 1 && mat2[time][column] == 1) {
          collector++;
        }
        if (collector > 0) {
          matrix[row][column] = 1;
